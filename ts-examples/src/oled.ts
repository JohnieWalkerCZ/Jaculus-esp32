...
export async function shapeExample() {
    ...
    const line1 = new LineSegment({ x: 0, y: 20, x2: 63, y2: 35, color: 0xffffff });
    scene.add(line1);

    const rect = new Rectangle({ x: 6, y: 6, width: 6, height: 6, color: 0xffffff, fill: true });
    scene.add(rect);

    const circle = new Circle({ x: 18, y: 6, radius: 5, color: 0xffffff, fill: true });
    scene.add(circle);

    const hexagon = new RegularPolygon({ x: 48, y: 48, radius: 10, sides: 6, color: 0xffffff, fill: true });
    scene.add(hexagon);

    const pentagon = new RegularPolygon({ x: 48, y: 16, radius: 8, sides: 5, color: 0xffffff, fill: true });
    scene.add(pentagon);

    const polygon = new Polygon({ x: 16, y: 48, color: 0xffffff, vertices: [[0, 0], [10, 5], [5, 15], [0, 10]], fill: true });
    scene.add(polygon);

    const line2 = new LineSegment({ x: 32, y: 0, x2: 20, y2: 40, color: 0xffffff });
    scene.add(line2);

    const lineRect = new Rectangle({ x: 25, y: 20, width: 8, height: 5, color: 0xffffff, fill: true });
    scene.add(lineRect);

    const point = new Point({ x: 32, y: 42, color: 0xffffff });
    scene.add(point);
